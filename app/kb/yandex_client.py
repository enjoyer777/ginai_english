...
# в xlsx умеют отдавать сломанный или пустой styles.xml — openpyxl падает с
# 'could not read stylesheet from None'. Подменяем его в zip-контейнере xlsx, и
# openpyxl уже не возражает.
...
def _sanitize_xlsx_styles(blob: bytes) -> bytes:
    """Перезаписывает xl/styles.xml внутри xlsx-zip на минимально валидный.

    Если исходный файл уже корректен — переписывание ничего не ломает,
    стили просто становятся «дефолтными». Для нашего use-case (читаем значения)
    это и нужно.
    """
    out = io.BytesIO()
        ...
        with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zout:
            ...
            for name in zin.namelist():
                if name == "xl/styles.xml":
                    zout.writestr(name, _MINIMAL_STYLES_XML)
                    replaced = True
                else:
                    zout.writestr(name, zin.read(name))
            if not replaced:
                zout.writestr("xl/styles.xml", _MINIMAL_STYLES_XML)
    return out.getvalue()
