...
**Лист `Праздники`** (опционально — особые даты-исключения, перекрывают недельное расписание)
...
- `hours` — пусто = нерабочий день; `HH:MM-HH:MM` = особый график для этой даты.
- `note` — человеческое описание (бот покажет клиенту: «9 мая — День Победы»).

Каждая запись на этом листе перекрывает обычное недельное расписание для своей даты. Лист можно не создавать — без него работает обычный недельный график.
